...
public class Client {
    ...
    private void loadConfiguration() {
        ...
        try (FileInputStream fis = new FileInputStream("client.conf")) {
            ...
            port = Integer.parseInt(config.getProperty("port", "23455"));
    ...
    private void downloadFile() {
        // Similar to sendFile, but adapted for receiving files in chunks from the sub-servers
        new Thread(() -> {
            try {
                ...
                // Send request to server to retrieve file
    ...
    private void deleteFile() {
        // Same as the previous deleteFile method
        new Thread(() -> {
            try {
    ...
    private void listFiles() {
        // Similar to the previous methods but requests the list of files from the main server
        new Thread(() -> {
            try {
                dos.writeUTF("GET_LIST");
                dos.flush();
